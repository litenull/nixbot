...
function ensureGroupDir(groupsDir: string, name: string): { groupPath: string; claudeMdPath: string } {
  const groupPath = path.join(groupsDir, name);
  fs.mkdirSync(groupPath, { recursive: true });
  const claudeMd = path.join(groupPath, "CLAUDE.md");
  if (!fs.existsSync(claudeMd)) {
    fs.writeFileSync(claudeMd, `# ${name} Group\n\nContext for this group.\n`);
  }
  return { groupPath, claudeMdPath: claudeMd };
}
...
await describe("repl utilities", async () => {
  ...
  await describe("truncateOutput", async () => {
    ...
    await it("truncates at exact boundary", async () => {
      ...
      // Result should be truncated with the suffix
      assert.ok(result.endsWith("\n... (truncated)"));
  ...
  await describe("isPauseInput", async () => {
    ...
    await it("rejects partial matches", async () => {
      assert.strictEqual(isPauseInput("paused"), false); // different word - ends with "ed"
      assert.strictEqual(isPauseInput("stopping"), false); // different word
      // "waiting room" contains "wait" but not as a word boundary, so it should NOT match
      assert.strictEqual(isPauseInput("waiting room"), false);
      // However, "please wait" should match because "wait" is at the end
      assert.strictEqual(isPauseInput("please wait"), true);
    });
