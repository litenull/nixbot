...
// Type guards for test database queries
interface CountRow {
  count: number;
}
...
function isMetadataRow(row: unknown): row is MetadataRow {
  return (
    typeof row === "object" &&
    row !== null &&
    ((row as Record<string, unknown>).metadata === null ||
      typeof (row as Record<string, unknown>).metadata === "string")
  );
}
...
function isDateRow(row: unknown): row is DateRow {
  return (
    typeof row === "object" &&
    row !== null &&
...
function isActionTypeRow(row: unknown): row is ActionTypeRow {
  return (
    typeof row === "object" &&
    row !== null &&
    typeof (row as Record<string, unknown>).action_type === "string"
  );
}
...
describe("Tape Logging", () => {
  ...
  describe("initTapeTable", () => {
    it("should create tape_log table", () => {
      const tables = db
        .prepare(
          "SELECT name FROM sqlite_master WHERE type='table' AND name='tape_log'",
        )
        .all();
    ...
    it("should create indexes", () => {
      ...
      function isIndexRow(row: unknown): row is IndexRow {
        return (
          typeof row === "object" &&
          row !== null &&
      ...
      const indexes = db
        .prepare(
          "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='tape_log'",
        )
        .all();
      const indexRows = indexes.filter(isIndexRow);
      const indexNames = indexRows.map((i) => i.name);
  ...
  describe("logTapeAction", () => {
    it("should log an action", () => {
      ...
      const countRow = db
        .prepare("SELECT COUNT(*) as count FROM tape_log")
        .get();
      assert.ok(isCountRow(countRow));
    ...
    it("should log action with metadata", () => {
      ...
      const row = db.prepare("SELECT * FROM tape_log").get();
      assert.ok(isMetadataRow(row));
    ...
    it("should set expiration date 30 days in future", () => {
      ...
      const row = db
        .prepare("SELECT created_at, expires_at FROM tape_log")
        .get();
      assert.ok(isDateRow(row));
    ...
    it("should log all action types", () => {
      const types: TapeEntry["actionType"][] = [
        "command",
        "output",
        "feedback",
        "llm_request",
      ...
      const rows = db
        .prepare("SELECT action_type FROM tape_log ORDER BY id")
        .all()
        .filter(isActionTypeRow);
  ...
  describe("cleanExpiredTapeEntries", () => {
    it("should remove expired entries", () => {
      ...
      const remaining = db
        .prepare("SELECT COUNT(*) as count FROM tape_log")
        .get();
      assert.ok(isCountRow(remaining));
