...
await describe("handleLiveFeedback", async () => {
  ...
  await it("returns LLM response on success", async () => {
    llmResponseBody = JSON.stringify({
  ...
  await it("includes context in supervisor prompt", async () => {
    let capturedBody = "";
    const capturingServer = createServer(
    ...
    const capPort = await new Promise<number>((resolve) => {
      capturingServer.listen(0, () => {
        const address = capturingServer.address();
        assert.ok(
          isAddressInfo(address),
          "Expected server address to be AddressInfo",
        );
    ...
    await handleLiveFeedback(
      ...
      { command: "npm install", partialOutput: "fetching packages..." },
      {
        ...
        llmConfig: {
          ...
          apiKey: "test",
          model: "m",
          baseUrl: `http://localhost:${capPort}`,
        },
        group: "work",
      },
    );
