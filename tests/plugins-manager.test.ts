...
await describe("plugin manager", async () => {
  await it("logs when no plugins are enabled", async () => {
    await withEnv(
      { NIXBOT_PLUGINS: undefined, NIXBOT_TELEGRAM_BOT_TOKEN: undefined },
      async () => {
        const logs: string[] = [];
        ...
        assert.equal(
          logs.includes("[plugins] No plugins enabled (set NIXBOT_PLUGINS)"),
          true,
        );
      },
    );
  ...
  await it("logs unknown plugins", async () => {
    await withEnv(
      {
        NIXBOT_PLUGINS: "unknown-plugin",
        NIXBOT_TELEGRAM_BOT_TOKEN: undefined,
      },
      async () => {
        const logs: string[] = [];
        const stop = await startPlugins(makeContext(logs));
        ...
        assert.equal(
          logs.includes("[plugins] Unknown plugin: unknown-plugin"),
          true,
        );
      },
    );
  ...
  await it("starts telegram plugin and skips when token is missing", async () => {
    await withEnv(
      { NIXBOT_PLUGINS: " TELEGRAM ", NIXBOT_TELEGRAM_BOT_TOKEN: undefined },
      async () => {
        const logs: string[] = [];
        const stop = await startPlugins(makeContext(logs));
        await stop();
