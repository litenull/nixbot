...
function buildEndpoint(baseUrl: string, _provider: string): string {
...
function maskCredentialsInOutput(
  output: string,
  credentials: Map<string, string>,
): string {
  let masked = output;
  for (const [_name, value] of credentials) {
...
`;
  ...
  await it("handles prompts with URLs and query params", async () => {
    const text =
      "/cron add test '0 * * * *' 'Check https://example.com?foo=bar&baz=qux'";
    const commands = extractCronCommands(text);
  ...
  await it("handles prompts with colons and spaces", async () => {
    const text =
      "/cron add report '0 9 * * *' 'Generate report: daily summary'";
    const commands = extractCronCommands(text);
...
await describe("message validation", async () => {
  ...
  await it("rejects non-string content", async () => {
    const messages = [{ role: "user", content: 123 as any }];
    const result = validateMessages(messages);
    assert.strictEqual(result.valid, false);
...
await describe("chat() HTTP integration", async () => {
  ...
  before(async () => {
    ...
    assert.ok(
      ...
      "Expected server address to be AddressInfo",
    );
  ...
  await it("uses default openai baseUrl when none provided", async () => {
    ...
    await assert.rejects(() =>
      chat({ provider: "openai", apiKey: "test", model: "gpt-4o" }, [
        { role: "user", content: "hi" },
      ]),
    );
  });
